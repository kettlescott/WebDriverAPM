...
import automation.web.base.elements.widget.SelectImpl;
import org.openqa.selenium.*;

import java.util.List;
import java.util.StringTokenizer;

public class HTMLSelectImpl extends SelectImpl implements HTMLSelect {
     ...
     */
    public HTMLSelectImpl(WebElement element) {
        super(element);
    ...
    public void selectByValue(String value) {
        ...
        HTMLSelectImpl.State state = HTMLSelectImpl.State.NOT_FOUND;
        for (WebElement option : options) {
            state = state.recognizeNewState(setSelected(option));
            if (!isMultiple() && state == HTMLSelectImpl.State.SELECTED) {
                return;
            }
        }
    ...
    /**
     * Select the option atPage the given index. This is done by examing the "index"
     * attribute of an element, and not merely by counting.
     *
     * @param index The option atPage this index will be selected
     * @throws NoSuchElementException If no matching option elements are found
     *             or the elements are not visible or disabled.
    ...
    private HTMLSelectImpl.State setSelected(WebElement option) {
        if (!option.isDisplayed()) {
            return HTMLSelectImpl.State.NOT_VISIBLE;
        }
        if (!option.isEnabled()) {
            return HTMLSelectImpl.State.DISABLED;
        }
        if (!option.isSelected()) {
            option.click();
        }
        return HTMLSelectImpl.State.SELECTED;
    }
    ...
    private enum State {
        ...
        private HTMLSelectImpl.State recognizeNewState(HTMLSelectImpl.State newState) {
    ...
    @Override
    public HTMLSelect refreshElement() {
        return (HTMLSelect) super.refreshElement();
    }
}
